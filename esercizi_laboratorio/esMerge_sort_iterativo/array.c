...
void mergeSortIterativo(int a[],int n)
{
	...
	while(cur < n)
	{
		for(int left_start = 0 ; left_start <  n ; left_start += 2 * cur)
		{
			int mid = min(left_start + cur, n);
			int right_end = min(left_start + 2 * cur, n);
			merge(a+left_start, a+mid, mid-left_start , right_end-mid ,a+left_start);
		}
...
void mergeSortStampe(int n)
{
	...
	{
		printf("dim : %d\n",cur);
		for(int left = 0 ; left + cur <  n ; left += 2 * cur)
		{
			int mid = left + cur -1;
			int right = min(left + 2*cur -1 , n-1);
			printf("merge di %d-%d e %d-%d\n",left,mid,mid+1,right);
			//printf("merge %d - %d\n",left_start,left_start + cur-1);
		}
